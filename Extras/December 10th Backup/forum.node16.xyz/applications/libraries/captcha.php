...
class TidyBB_Captcha
{
    ...
    public function display()
    {
        ...
        if ($this->captcha_type == "1") {
            return '<img src="' . SITE_URL . '/public/img/captcha.php" alt="TidyBB Captcha" /><br /><input type="text" id="TidyBB_captcha" name="TidyBB_captcha" />';
        } else {
    ...
    public function verify()
    {
        global $LANG;
        if ($this->captcha_type == "1") {
            $input = md5($_POST['TidyBB_captcha']);
            if ($input !== $_SESSION['TidyBB_Captcha']) {
